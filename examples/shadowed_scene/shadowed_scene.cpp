...
int main(int argc, char **argv) 
{
  ...
  auto concrete_texture = std::make_shared<Image2D<uint>>(
      LoadImage<uint>((rsc_path+"concrete.bmp").data()));
  auto box_texture = std::make_shared<Image2D<uint>>(
      LoadImage<uint>((rsc_path+"box.bmp").data()));
  ...
  auto tex_floor_shader = std::make_shared<TexturedShader>(
      floor.vertices.data(), 
      floor.tex_coords.data(), 
      floor.indices.data());
  ...
  auto tex_cubes_shader = std::make_shared<TexturedShader>(
      cubes.vertices.data(), 
      cubes.tex_coords.data(), 
      cubes.indices.data());
  ...
  auto sm_cubes_shader = std::make_shared<ShadowMapShader>(
      cubes.vertices.data(), 
      cubes.indices.data());
  sm_cubes_shader->view = light_view;
  sm_cubes_shader->projection = projection;

  auto tex_floor_rasterizer = make_cpu_vsps_shader<TexturedShader>(argv[0], 
                                                                   tex_floor_shader);
